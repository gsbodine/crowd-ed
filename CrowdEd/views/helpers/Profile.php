...
/* PRIVATE FUNCTIONS */
    private function _selectUserItems($user,$relationshipName='modified',$icon='icon-circle',$class='label label-inverse',$metadata=array('Dublin Core','Title'),$limit=null,$type='list') {
        ...
        foreach ($stmt as $row) {
            ...
            if ($type == 'table') {
                $html .= '<tr><td>'. link_to_item(item_image('thumbnail', array(), 0, $item), array('class' => 'img'), 'show', $item) .'</td><td><p class="lead">'. link_to_item(metadata($item, array('Dublin Core','Title')), array('class' => 'title'), 'show', $item).'</p><p class="well">'. metadata($item, array('Dublin Core','Description')) .'</p><p>'. metadata($item,'citation',array('no_escape' => true)) .'</p></td><td><p class="text-center"><b><i class="icon-calendar"></i> &ndash; '. date("M j, Y",strtotime($row['time'])). '<br /><i class="icon-time"></i> &ndash; '. date("g:i:s a",strtotime($row['time'])) .'</b></p></td></tr>';
            } else if ($type == 'list') {
